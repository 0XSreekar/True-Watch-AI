...
(`MEASUREMENTS.md`, 31,569 / 10,225 / 4,794). Its detection taxonomy has 15 classes. Every one is
...
### 1.3 KAIST Multispectral Pedestrian — all native classes mapped

KAIST annotates pedestrians only, in its own text format, with a small tag vocabulary.
...
change under either outcome. This closes `PHASE_MINUS1_SCOPE` open question 1 with a decision
...
- KAIST: `sequence_key = "kaist/<setNN>/<VNNN>"` — the set and video folder from the native layout.
- LLVIP: `sequence_key = "llvip/<scene_prefix>"` — see §2.3 and open question **OQ-3**.
- IDD: `sequence_key = "idd/<drive_folder>"` — IDD's images are organised under per-drive folders,
  so a drive is the sequence.
...
### 2.2 KAIST — explicit set assignment
...
- **LLVIP** ships an official train/test partition. The official test partition becomes part of our
  **held-out test** untouched. Our **val** is carved out of the official train partition by scene
  key, never by random frame. LLVIP's scene grouping is **OQ-3**: if the filename prefix does not
  yield a reliable scene key at runtime, `06_split.py` falls back to grouping by perceptual-hash
  adjacency (Hamming distance <= 6 on a 64-bit pHash forms one pseudo-sequence) and logs that it
  did so. The fallback is conservative — it over-groups, which costs split size but cannot leak.
- **IDD** ships train/val/test, but the public IDD test partition has no released labels. So: IDD
  train -> our **train**; IDD val is split by drive folder into our **val** and **test**; IDD test
  is not used at all (no labels, no value here).
...
selected by four objective criteria:
...
member list is frozen in `datasets/manifests/hard_set.txt`.
...
**Prediction.** `person` dominates. KAIST and LLVIP are pedestrian-only corpora contributing
roughly 38k labelled frames between them; IDD is the only vehicle source. Expected instance
...
**Prediction.** Bimodal. IDD is dashcam-framed, so its vehicles are large; KAIST/LLVIP pedestrians
...
**Prediction.** IDD is daylight-only. KAIST is roughly half night. LLVIP is predominantly night.
Combined, the raw corpus lands near 60:40 visible:infrared — which is inside the §2.4 tolerance by
luck rather than design, so it is asserted as a gate (G8), not assumed.
...
3. **Replicate the single channel across all three** — `np.repeat(ch[..., None], 3, axis=2)` — so
   ...
   colourising.
4. Write as 3-channel PNG so the stored artefact *is* 3-channel and gate G9 can verify it with
   `cv2.imread(p).shape`.
...
| Gaussian noise | σ 2–8 (8-bit) | uncooled microbolometers are noisy; KAIST and LLVIP are cleaner than a deployed camera. |
...
it is a real deployment variable, but neither KAIST nor LLVIP documents its polarity convention per
...
| Headlight glare, night blooming | a blown-out headlight is person-sized and person-bright on an 8-bit frame | KAIST and LLVIP **night** frames with zero annotations. Free, already in the corpus. |
...
|---|---|---|---|
| **IDD Detection** | 46,588 images (31,569 / 10,225 / 4,794), IIIT Hyderabad | Non-commercial research use, behind registration and a click-through agreement at `idd.insaan.iiit.ac.in`. **The exact clause text is OQ-4** — it is read and quoted at fetch time, not paraphrased from memory. | **Assume NO.** `fetch_idd.py` already refuses to bypass the gate; nothing derived from IDD is re-hosted. |
| **LLVIP** | 15,488 visible/infrared pairs (**OQ-5** — the pair count is the commonly cited figure and is verified at fetch time against the actual file count), Hugging Face `jsonhash/LLVIP` | Research use; the upstream repository states its own terms. **OQ-5.** | **Assume NO** until the dataset card on the Hub is read at fetch time. |
| **KAIST Multispectral Pedestrian** | 23,210 files (`MEASUREMENTS.md`), Hugging Face `richidubey/KAIST-...` | Research use, KAIST/CVPR origin. **OQ-5.** | **Assume NO.** |
...
- Geographic: IDD is Indian urban/semi-urban roads; KAIST is Korean campus and downtown; LLVIP is
  Chinese street scenes. **No source is an Indo-Nepal border post.** Every number this corpus
  produces is a public-dataset number, exactly as slide 5's status line already says.
- Viewpoint: IDD is dashcam height; KAIST and LLVIP are near-eye-level. **None is pole-mounted at
  4–6 m looking down a 150 m approach.** §3.3 tiling and §4.1 downscale augmentation are partial
  compensations, not a fix.
- Taxonomy impurity: KAIST `cyclist` boxes enter as `person` and include the bicycle (§1.3).
...
| G8 | Day/night ratio | visible:infrared per split from the source manifest | each split between **55:45 and 70:30** |
| G9 | Negative ratio | zero-byte label files in **train** | **10% ± 2%** (i.e. 8.0–12.0%) |
| G10 | IR frames are genuinely 3-channel | `cv2.imread(p).shape` on a random **500**-image IR sample | **100%** report `(_, _, 3)`; **0** failures |
...
| G12 | Box-size histogram present with a small-object tail | height histogram in px over 8 bins | file exists **and >= 5%** of person boxes fall below **19 px** — the corpus must actually contain the hard band it claims to train for |
| G13 | Far-field tiles present | tiled images in train | **>= 2,000** tiles, each 640×640 exactly |
| G14 | Split proportions | image counts | train **70% ± 3**, val **15% ± 3**, test **15% ± 3** |
| G15 | Hard set frozen and disjoint from test | membership of `hard_set.txt` | exactly **280** entries, **0** of which are in the test split |
...
Nineteen gates, every one with a number. G19 is deliberately not automatable — a human looking at
...
- **OQ-1 — KAIST set lighting composition.** The §2.2 assignment assumes set00–02 day, set03–05
  ...
  assumption.
- **OQ-2 — IDD directory layout.** Whether IDD Detection's annotations are per-image XML in a
  parallel tree or a consolidated index is not assumed; `01_convert_idd.py` detects the layout at
  runtime and logs which it found.
- **OQ-3 — LLVIP scene grouping.** Whether filenames encode a recoverable scene id. Fallback in
  §2.3 is pHash pseudo-sequences.
...
- **OQ-6 — infrared polarity convention** per sequence in KAIST and LLVIP. Polarity inversion stays
...
- **OQ-11 — the cart outcome.** Resolved by running the §1.5 procedure, not by discussion.
...
- [x] **Every native class of IDD, LLVIP and KAIST is mapped or explicitly dropped.** IDD: all 15
      ...
      KAIST: 4 tags in §1.3 — 2 mapped, 2 to ignore regions. LLVIP: 1 class, mapped. None
      unaccounted.
...
- [x] **Every licence claim is sourced or marked OPEN QUESTION.** IDD gate → OQ-4; LLVIP and KAIST →
