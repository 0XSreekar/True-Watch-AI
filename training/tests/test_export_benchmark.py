...
IMGSZ = 160
PARITY_LINE = re.compile(r"^PARITY max_abs_diff=\d\.\d{3}e[-+]\d{2} tolerance=1e-03 -> (PASS|FAIL)$", re.M)
...
class FakeHfApi:
    """Records what would be sent to the Hub."""
    ...
    def upload_file(self, path_or_fileobj, path_in_repo, repo_id, repo_type="model", **kwargs):
        ...
        self.uploads.append({"path_in_repo": path_in_repo, "repo_id": repo_id, "bytes": len(content), "content": content})
...
def test_real_export_passes_parity_and_prints_the_exact_line(exported):
    ...
    assert re.search(r"^PARITY max_abs_diff=\S+ tolerance=1e-03 -> PASS$", exported["stdout"], re.M)
    assert re.search(r"batch=1 .*boxes\(norm\)=\S+ +scores=\S+", exported["stdout"])
    assert re.search(r"batch=3 .*boxes\(norm\)=\S+ +scores=\S+", exported["stdout"])
...
def test_json_follows_the_export_schema(exported):
    ...
    # Sensitivity is measured in the same units as the tolerance (normalized boxes by default), so the
...
def test_an_input_insensitive_model_is_flagged_as_a_weak_check(export_stack, tmp_path, capsys, guarded_hub):
    ...
    assert out.index("WEAK CHECK") < out.index("PARITY max_abs_diff")  # and the PARITY line stays last
...
def test_float32_noise_is_told_apart_from_a_graph_error(tmp_path, capsys, guarded_hub):
    ...
    assert "below the float32 noise floor" in captured.err and "--box-units normalized" in captured.err
    assert re.search(r"^PARITY max_abs_diff=\S+ tolerance=1e-03 -> FAIL$", captured.out, re.M)

    again = E.main(_args(tmp_path, noisy, "--tag", "noisy-norm"))  # normalized is the default
    out = capsys.readouterr().out
    normalized = json.loads((tmp_path / "r" / "export_noisy-norm.json").read_text(encoding="utf-8"))["parity"]
    assert again == E.EXIT_OK and normalized["box_units"] == "normalized" and normalized["passed"] is True
    assert "boxes(norm)=" in out and "fractions of the input size" in out
    assert normalized["max_abs_diff_scores"] == pytest.approx(record["parity"]["max_abs_diff_scores"], rel=0.5)  # scores are not rescaled
...
def test_failed_parity_exits_1_writes_evidence_and_never_uploads(export_stack, tmp_path, monkeypatch, capsys, guarded_hub):
    ...
    assert re.search(r"^PARITY max_abs_diff=\S+ tolerance=0e\+00 -> FAIL$", captured.out, re.M)
...
def test_push_to_hub_uploads_onnx_and_card_and_records_the_url(export_stack, tmp_path, monkeypatch, capsys):
    import huggingface_hub

    FakeHfApi.instances.clear()
    monkeypatch.setattr(huggingface_hub, "HfApi", FakeHfApi)
    monkeypatch.setenv("HF_TOKEN", "secret-token-value")
    code = E.main(_args(tmp_path, export_stack["weights"], "--tag", "hub", "--push-to-hub", "someone/truewatch-detector", "--private"))
    captured = capsys.readouterr()
    assert code == E.EXIT_OK
    (api,) = FakeHfApi.instances
    assert api.token == "secret-token-value"
    assert api.repos == [{"repo_id": "someone/truewatch-detector", "repo_type": "model", "private": True, "exist_ok": True}]
    assert sorted(u["path_in_repo"] for u in api.uploads) == ["README.md", "hub.onnx"]
    assert next(u for u in api.uploads if u["path_in_repo"] == "hub.onnx")["bytes"] == (tmp_path / "o" / "hub.onnx").stat().st_size
    record_text = (tmp_path / "r" / "export_hub.json").read_text(encoding="utf-8")
    assert json.loads(record_text)["hf"] == {"repo_id": "someone/truewatch-detector", "url": "https://huggingface.co/someone/truewatch-detector"}
    assert "secret-token-value" not in record_text + captured.out + captured.err
...
def test_parity_line_format_is_exact():
    assert E.parity_line(1.5e-5, 1e-3, True) == "PARITY max_abs_diff=1.500e-05 tolerance=1e-03 -> PASS"
    assert E.parity_line(2.5e-3, 1e-3, False) == "PARITY max_abs_diff=2.500e-03 tolerance=1e-03 -> FAIL"
